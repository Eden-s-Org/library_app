...
const SingleBookCard = ({ title, author, image, isbn, category, yearofpublication, pages, description }) => {
    // Function to handle delete
    const handleDelete = async () => {
        const confirmDelete = window.confirm(`Are you sure you want to delete "${title}"?`);
        if (confirmDelete) {
            try {
                await axios.delete(`https://library-api-gxyy.onrender.com/api/v1/books/${id}`);
                alert("Book deleted successfully!");
                onDelete(id); // Update parent state after deletion
            } catch (error) {
            ...
            <div className="h-screen flex flex-col justify-center">
                <div class="bg-gray-200 grid grid-cols-1 md:grid-cols-2 md:mt-8 mx-auto w-[90%] shadow-lg shadow-blue-950 rounded-2xl justify-center relative overflow-hidden">
                    ...
                    <div class="flex flex-col justify-evenly">
                        ...
                        <h2 class="text-2xl md:text-5xl/20 font-bold text-blue-950 mt-7 ">{title}</h2>

                        <div className="flex">
                            <p className="text-blue-950 font-semibold text-sm md:text-md">Authored by:</p>
                            <p className=" text-blue-950 ml-2">{author}</p>
                        </div>
                ...
                italic text-blue-950'>{isbn}</span></p>
                        ...
                        <p className=''><span className='text-blue-950 font-semibold text-sm md:text-md'>Book Category:</span> <span className='text-blue-950'>{category}</span></p>
